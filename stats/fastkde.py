...
2022 12 13: Version 1.7: Fix issues with taking the log of zero.
"""
...
class KDE:
        ...
        if not self.constants.variable_bandwidth:
            if not self.weights:
                if self.constants.ndata > 10000:
                    ...
                    score = np.sum(np.exp(self.data_helpers.mindists[:self.constants.ndata,
    ...
    def score_samples(self, xdata: np.ndarray = None) -> np.ndarray:
        ...
        if len(xdata.shape) == 1:
            xdata = xdata[:, np.newaxis]
        if len(xdata.shape) == 2:
            return self._score_samples(xdata)
        ...
        scores = self._score_samples(xdata.reshape((np.prod(newshape), xdata.shape[-1])))
        return scores.reshape(newshape)

    def _score_samples(self, xdata: np.ndarray = None) -> np.ndarray:
        """ Return the scores, i.e., the value of the pdf, for all the datapoints in x.
