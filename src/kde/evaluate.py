...
def uv_evaluation_pipeline(path: Path):
    for f in tqdm(list(path.glob("**/*"))):
        ...
        #     improved_eps)
        true, baseline_jaccard_mean, baseline_jaccard_std = evaluate_epsilon_support(
            baseline_eps
        )
        _, improved_jaccard_mean, improved_jaccard_std = evaluate_epsilon_support(
            improved_eps
        )
        baseline_mse, baseline_mean, baseline_std, _, _ = evaluate_pdf(baseline_df)
        improved_mse, improved_mean, improved_std, _, _ = evaluate_pdf(improved_df)
        ...
        utils.save_csv(
            path / "results" / (f.name + ".results.csv"),
            pd.DataFrame(
                {
                    "x": baseline_df["x"],
        ...
        utils.save_json(
            path / "results" / (f.name + ".eps.results.json"),
            {
                # 'true_eps': true,
