...
def main(baseline_path, improved_path):
    baseline_model = FeedForward.load_from_checkpoint(baseline_path, num_in=1, num_hidden=uv_params.nn_num_hidden_nodes, num_out=1, num_layers=uv_params.nn_num_hidden_layers)
    improved_model = FeedForward.load_from_checkpoint(improved_path, num_in=1, num_hidden=uv_params.nn_num_hidden_nodes, num_out=1, num_layers=uv_params.nn_num_hidden_layers)
    ...
    axs[0].plot(x_values, uv_params.single_distributions_x1['bivariate_guassian_b'].cdf(x_values), label='true', linestyle='dotted')
    axs[0].plot(x_values, base_cdf_hat, label='baseline', alpha=0.5)
    axs[0].plot(x_values, imp_cdf_hat, label='improved', alpha=0.5)
    axs[0].legend()
    
    axs[1].plot(x_values, uv_params.single_distributions_x1['bivariate_guassian_b'].pdf(x_values), label='true', linestyle='dotted')
