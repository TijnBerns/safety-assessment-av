...
class Evaluator:
    ...
    def _initialize_dataset(
        self, dataset: data.base.CustomDataset, test_set: str
    ) -> data.base.CustomDataset:
    ...
    def _initialize_dataloaders(
        self, dataset: data.base.CustomDataset, args: parameters.Parameters
    ) -> DataLoader:
    ...
    def _load_checkpoint(self, checkpoint: Path) -> FlowModule:
        ...
        try:
            ...
            flow_module = FlowModuleTrainableWeight.load_from_checkpoint(
                checkpoint, config=config, map_location="cpu"
            ).eval()
    ...
    def compute_llh(self, llh_estimate: Path):
        ...
        return (
            float(torch.mean(all).item()),
            float(torch.mean(non_event).item()),
            float(torch.mean(event).item()),
        )
    ...
    def compute_lr(self, true: Path, llh_estimate: Path) -> Tuple[float, float]:
        ...
        # Split in all, non-event, and event
        all_estimate, non_event_estimate, event_estimate = self._split_llh(
            llh_estimate, data
        )
...
@click.option(
    "--test_set",
    default="sampled",
    help="Whether to evaluate on test data normalized using all data or normal data only. Choices: [normal, all]",
)
