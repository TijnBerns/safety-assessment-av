...
def create_data_loaders(
    dataset: CustomDataset, batch_size: int, dataset_type: str
    ...
    raise ValueError(
        f"expected dataset_type one of: all, normal, weighted, sampled_normal, or sampled_weighted but got {dataset_type}"
    )
...
def create_module(
    ...
    if dataset_type in ["weighted", "sampled_weighted"]:
        return FlowModuleWeighted(
            features=features, dataset=dataset, args=args, stage=stage, weight=weight
        )
    else:
        return FlowModule(
            features=features, dataset=dataset, args=args, stage=stage, weight=weight
...
def train(dataset: str, dataset_type: str, weight: float):
    ...
    if weight is None:
        ...
        weight = data.preprocess.compute_event_weight(
            normal=normal_data,
            event=event_data,
            xi=dataset().xi,
