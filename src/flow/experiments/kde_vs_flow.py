    ...
    for num_dim in N_DIM:
        ...
        for num_samples in N_SAMPLES:
            for _ in range(N_MODELS):
                res = {"num_dim": num_dim, "num_samples": num_samples}
                ...
                # Fit Flow
                flow = FlowModule(
                    features=num_dim, dataset=dataset(), stage=2, args=args
                )
                ...
                results.append(res)
                utils.save_json(
                    path=Path(f"kde_vs_flow_{seed}.json"), data={"data": results}
                )
