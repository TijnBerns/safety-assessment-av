...
def create_checkpointer(prefix: str = None):
    if prefix is None:
        ...
        pattern = (
            prefix
            + ".epoch_{epoch:04d}.step_{step:09d}.log_density_{val_log_density:.2f}"
        )
...
def create_data_loaders(dataset: CustomDataset, batch_size: int, dataset_type: str):
    val = DataLoader(
        dataset(split="val"), shuffle=False, batch_size=batch_size, num_workers=2
    )

    if dataset_type == "all":
        train = DataLoader(
            dataset(split="_train"), shuffle=True, batch_size=batch_size, num_workers=2
        )
        val = DataLoader(
            dataset(split="_val"), shuffle=False, batch_size=batch_size, num_workers=2
        )
        return train, None, val

    elif dataset_type == "split":
        train_normal = DataLoader(
            dataset(split="normal_train"),
            shuffle=True,
            batch_size=batch_size,
            num_workers=2,
        )
        train_event = DataLoader(
            dataset(split="event_train"),
            shuffle=True,
            batch_size=batch_size,
            num_workers=2,
        )
        ...
        train_normal = DataLoader(
            dataset(split="normal_sampled"),
            shuffle=True,
            batch_size=batch_size,
            num_workers=2,
        )
        train_event = DataLoader(
            dataset(split="event_sampled"),
            shuffle=True,
            batch_size=batch_size,
            num_workers=2,
        )
        val = DataLoader(
            dataset(split="val_sampled"),
            shuffle=False,
            batch_size=batch_size,
            num_workers=2,
        )
...
def create_module(dataset, features, dataset_type, args, stage):
    if dataset_type in ["weighted", "sampled_weighted"]:
        return FlowModuleWeighted(
            features=features, dataset=dataset, args=args, stage=stage
        )
    else:
...
@click.option(
    "--dataset_type", type=str, default="split"
)  # choices=['weighted','all','split', 'zero_weight']
def train(pretrain: bool, dataset: str, dataset_type: str):
    ...
    normal_train, event_train, val = create_data_loaders(
        dataset, args.batch_size, dataset_type
    )
    event_data = event_train.dataset.data
    xi = dataset().xi
    threshold = dataset().threshold
    event_from_normal = normal_train.dataset.data[
        normal_train.dataset.data[:, xi] > threshold
    ]
    event_train = DataLoader(
        np.concatenate((event_data, event_from_normal)),
        shuffle=True,
    ...
    n_event = np.sum(
        normal_train.dataset.data[:, normal_train.dataset.xi]
        > normal_train.dataset.threshold
    )
    flow_module = FlowModuleSampled(
        features=features,
        dataset=dataset(),
        args=args,
        stage=1,
        n_event=n_event,
        event_loader=event_train,
    )
    ...
    if pretrain:
        ...
        trainer_stage_1 = pl.Trainer(
            max_steps=args.training_steps_stage_1,
        ...
        args.training_steps_stage_2 = (
            args.training_steps_stage_1 + args.training_steps_stage_2
        )
    ...
    trainer_stage_2 = pl.Trainer(
        max_steps=args.training_steps_stage_2,
        inference_mode=False,
