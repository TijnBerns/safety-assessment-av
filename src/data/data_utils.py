...
def determine_threshold(
    frac_edge: float, distribution: scipy.stats.rv_continuous = scipy.stats.norm()
) -> float:
...
def generate_data(
    ditribution, num_norm, num_edge, threshold, dim: int = -1, random_state=0
    ...
    while True:
        ...
        edge_data = (
            np.concatenate((edge_data, new_edge_data))
            if edge_data.size
            else new_edge_data
        )
...
def filter_data(
    normal_data: np.ndarray, edge_data: np.ndarray, threshold: float = 0, dim=-1
) -> Tuple[np.ndarray, np.ndarray]:
...
def combine_data(
    ...
    repeat_factor = (
        (1 - p_edge)
        * (len(edge_from_normal_data) + len(edge_data))
        / (p_edge * len(normal_data_only))
    )
    ...
    combined_data = np.concatenate(
        (combined_data, normal_data_only[:M], edge_data, edge_from_normal_data)
    )
