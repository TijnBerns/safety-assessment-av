...
#     fig, axes2d = plt.subplots(nrows=len(datasets), ncols=3,sharex=False, sharey=False,figsize=FIGSIZE)
    
#     for i, row in enumerate(axes2d):
#         normal = datasets[i](split='normal_train_unnormalized')
#         event = datasets[i](split='event_train_unnormalized')
#         normal_summary = print_summary(normal)
#         event_summary = print_summary(event)
        
#         for j, cell in enumerate(row):
#             boxplot(normal_summary[j], event_summary[j], cell)
            
#             if i == len(axes2d) - 1:
#                 cell.set_xlabel(f"{column_labels[j]}")
#             if j == 0:
#                 cell.set_ylabel(f"{normal.name.upper()}") 
    
#         plt.tight_layout()
#         plt.savefig('img/summary_statistics_box.pgf')
...
#         plt.tight_layout()
#         plt.savefig('img/summary_statistics_scatter.pgf')
