"""
Module containing HEPMASS dataset class used in normalizing flow experiments.
"""
...
class Hepmass(CustomDataset):
    ...
    def load_data(self):
        ...
        train = train[
            :,
            np.array([i for i in range(train.shape[1]) if i not in features_to_remove]),
        ]
        test = test[
            :,
            np.array([i for i in range(test.shape[1]) if i not in features_to_remove]),
        ]
